...
public class app {
	...
	public void createNewBooking(String initialDate, String finalDate, String guestName, String guestMail,
			String guestPhone, Integer id2) {
	...
	public List<Integer> getRoomsAvailableBasic(String initialDate, String finalDate, String typeroom,
			Boolean kitchen_property, Boolean view_property, Boolean balcony_property, String bed_types, 
			Integer num_beds) {
		...
		List<Room> booked_rooms_in_date = inventory.getDisponibilityRoom(initialDate, finalDate);
		 ...
		 for (Room room : inventory.getRooms()) {
			 Integer number_of_beds = room.getBeds().size();
			 String bed_type = room.getBeds().get(0).getSize();
			 if (booked_rooms_in_date.contains(room) != true && room.getType().equals(typeroom) &&
					 room.getBalcony() == balcony_property && room.getView() == view_property &&
					 room.getKitchen() == kitchen_property && number_of_beds == num_beds &&
					 bed_type.equals(bed_types)) {
	...
	public List<Integer> getRoomsAvailableAdvanced(String initialDate, String finalDate, String typeroom,
			Boolean kitchen_property, Boolean view_property, Boolean balcony_property, String bed_types, 
			Integer num_beds, Boolean tv_property, Boolean heating_property, Boolean air_conditioner_property,
			Boolean coffee_maker_property, Boolean dryer_property, Boolean hair_iron_property, 
			Boolean ac_voltage_property, Boolean usb_a_property, Boolean usb_c_property, 
			Boolean breakfast_property, String meters_square) {
		List<Integer> availableRooms = new ArrayList<>();
		List<Room> booked_rooms_in_date = inventory.getDisponibilityRoom(initialDate, finalDate);
		 List<Room> Provisional_Rooms = new ArrayList<>();
		 for (Room room : inventory.getRooms()) {
			 ...
			 if (booked_rooms_in_date.contains(room) != true && room.getType().equals(typeroom) &&
					 ...
					 room.getKitchen() == kitchen_property && number_of_beds == num_beds &&
					 bed_type.equals(bed_types)) {
				 String[] parts = meters_square.split("_");
					Integer min_size = Integer.parseInt(parts[0]);
					Integer max_size = Integer.parseInt(parts[1]);
				 //new advanced characteristics
				 if(tv_property == room.getAdvancedCharacteristic("tv") && 
						 heating_property == room.getAdvancedCharacteristic("heating") && 
						 air_conditioner_property == room.getAdvancedCharacteristic("air_conditioner") &&
							coffee_maker_property == room.getAdvancedCharacteristic("coffee_maker") && 
							dryer_property == room.getAdvancedCharacteristic("dryer") && 
							hair_iron_property == room.getAdvancedCharacteristic("hair_iron" ) &&
							ac_voltage_property == room.getAdvancedCharacteristic("ac_voltage") &&
							usb_a_property == room.getAdvancedCharacteristic("usb_a") && 
							usb_c_property == room.getAdvancedCharacteristic("usb_c") &&
							breakfast_property == room.getAdvancedCharacteristic("breakfast") &&
							min_size <= room.getM2() && room.getM2()<= max_size) {
				 Provisional_Rooms.add(room);
				 }
			 }//end if
		 }// final for of searching rooms
